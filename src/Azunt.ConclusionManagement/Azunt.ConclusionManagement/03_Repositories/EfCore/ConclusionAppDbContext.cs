...
/// <summary>
/// ConclusionApp에서 사용하는 데이터베이스 컨텍스트 클래스입니다.
/// EF Core In-Memory 테스트와 SQL Server 운영 환경에서 모두 사용할 수 있습니다.
/// </summary>
public class ConclusionAppDbContext : DbContext
{
    /// <summary>
    /// DbContextOptions을 인자로 받는 생성자입니다.
    /// 주로 Program.cs 또는 Startup.cs에서 서비스로 등록할 때 사용됩니다.
    /// </summary>
    public ConclusionAppDbContext(DbContextOptions<ConclusionAppDbContext> options)
        : base(options)
    {
    ...
    /// <summary>
    /// ConclusionApp 관련 테이블을 정의합니다.
    /// </summary>
    public DbSet<Conclusion> Conclusions { get; set; } = null!;
}
